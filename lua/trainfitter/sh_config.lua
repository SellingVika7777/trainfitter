...
if CLIENT then
    ...
    --# слабый ПК ? Ставь 0 - клиент не качает и не маунтит скины, составы будут с дефолтным видом
...
function Trainfitter.SafeFetchWorkshopInfo(wsid, cb)
    ...
    http.Post(
        ...
        function(body)
            --# не доверяем размеру ответа (вдруг MITM/спуф) - режем до парса, реальные ответы это килобайты
...
--# CAMI - общий интерфейс админок. Регнул прайвилегии тут - они сами всплывут в ULX/SAM/ServerGuard/Maestro и т.д.
...
--# ПУБЛИЧНЫЙ API: любая (в т.ч. самописная) админка регает свою проверку прав
--# fn(ply, privilege) -> true (пускаем) / false (мнение "нет") / nil (без мнения)
--# любой true побеждает (OR), как было раньше. Порядок = порядок регистрации
...
--# встроенные адаптеры. Регаем один раз, в том же порядке что и была старая цепочка
if not Trainfitter._builtinProvidersRegistered then
    ...
    Trainfitter.RegisterPermissionProvider("lfadmin", function(ply, priv)
        return LFAdminHasAccess(ply, priv) --# true / nil
    end)

    Trainfitter.RegisterPermissionProvider("ulx", function(ply, priv)
        return ULXHasAccess(ply, priv) --# true / nil
    end)

    --# evolve - нативная, не CAMI. Только grant, без жёсткого deny
    ...
    --# CAMI - накрывает ULX/SAM/ServerGuard/Maestro/FAdmin и прочих кто его поддерживает
    --# Fallback="no_one" чтобы дефолтный обработчик НЕ раздавал права когда админки нет (это работа нашего fallback ниже)
    Trainfitter.RegisterPermissionProvider("CAMI", function(ply, priv)
        ...
        if not decided then return nil end --# админка ответила async - не нам решать
        if result then return true end
        return nil --# CAMI сказал нет - не жёсткий deny, пусть решает наш fallback
    end)
end
...
local function CheckPrivilege(ply, accessName, fallback)
    if not IsValid(ply) then return false end
    if IsServerOwner(ply) then return true end --# хост listen-сервера / синглплеер - всегда босс

    for _, p in ipairs(Trainfitter._PermProviders) do
        --# pcall чтобы кривой самописный провайдер не уронил всю проверку
        local ok, res = pcall(p.fn, ply, accessName)
        if ok and res == true then return true end --# любой grant побеждает
    end

    return fallback(ply) == true --# админки молчат - дефолт гмода (IsAdmin / IsSuperAdmin)
end
...
function Trainfitter.CanViewLogs(ply)
    if Trainfitter.CanManage(ply) then return true end --# суперадмин-менеджер видит логи всегда
