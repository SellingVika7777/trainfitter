...
--# освобождаем слоты ИГРОКА под его новый скин (FIFO по его же старым), чужие скины не трогаем
...
local function SetActiveSkin(wsid, title, sizeMB, initiatorName, initiatorSid)
    --# ActiveSkin теперь чисто инфо для UI (последний применённый), БЕЗ авто-забывания старого
    --# раньше тут старый скин уходил в утиль у ВСЕХ - вот и был баг "один скин на всех"
...
--# считаем и уже загруженные, и те что ещё качаются (pending), иначе пачка коллекции пробьёт лимит
...
local function HandleRequest(ply, wsid, makePersistent, skipCooldown)
    ...
    --# лимиты загрузки (0 = безлимит), re-apply уже загруженного или качающегося wsid не считается
    ...
    if not alreadyLoaded then
        --# общий потолок сервера - жёсткий (личный лимит решается заменой своего старого, см FinalizePending)
...
function Trainfitter.FinalizePending(wsid, ok, reason)
    ...
    --# новый скин загрузился - теперь подрезаем СВОИ старые под личный лимит (чужие не трогаем)
...
net.Receive(NET.Request, function(len, ply)
    --# throttle ПЕРВЫМ - иначе спам oversized-пакетами пишет в audit.log без лимита (disk DoS)
...
net.Receive(NET.RequestCollection, function(len, ply)
    ...
    --# отдельный кулдаун на коллекции - одна коллекция кидает пачку запросов, без него можно забить очередь
    ...
    --# allow_collections рулит сервер, клиент тут не указ
    ...
    Trainfitter.FetchCollectionChildren(wsid, function(children, err)
        ...
        --# синхронно режем по свободным слотам - pending выставляется async, иначе пачка пробьёт лимит
        local sid         = ply:SteamID64() or "0"
        ...
        --# раскидываем по таймеру чтобы не задушить Steam API, кулдаун обходим (это одно согласованное действие)
        for i = 1, total do
...
net.Receive(NET.ReportSkins, function(len, ply)
    ...
    --# репортить владение скином может ТОЛЬКО его инициатор или админ - иначе чужой подделает (poisoning)
    --# авторитетный источник владения и так серверный diffInto в ServerExecuteSkinFiles, это лишь хинт
...
--# игрок снова включил скины - шлём ему ВСЕ текущие (и persistent, и session), чтоб подтянулись без релога
...
local function SendAdminConfig(target)
    ...
    if canManage then
        ...
        net.WriteUInt(0, 8) --# не-менеджеру значения convar'ов незачем светить
    end
    net.Send(target)
end
...
net.Receive(NET.AdminGetConfig, function(_, ply)
    if not IsValid(ply) then return end
    --# логи-админ (не суперадмин) тоже должен получить конфиг, иначе вкладка Логи не покажется
    ...
    SendAdminConfig(ply)
    if Trainfitter.CanManage(ply) then SendAdminLists(ply) end --# списки остаются только менеджерам
end)
...
net.Receive(NET.GetLogs, function(_, ply)
    if not IsValid(ply) then return end
    if not Trainfitter.CanViewLogs(ply) then return end --# серверная проверка - авторитетная
    local sid = ply:SteamID64() or "0"
    local now = CurTime()
    ...
    --# клампим страницу в разумный диапазон (OOB и так ловится ниже, но пусть будет чисто)
    local maxPage = total > 0 and math.ceil(total / perPage) - 1 or 0
    if page > maxPage then page = maxPage end

    --# страница 0 = самые свежие. Идём от хвоста к началу
...
--# перерегаем CAMI-прайвилегии после загрузки геймода (FAdmin/DarkRP грузят CAMI поздно)
