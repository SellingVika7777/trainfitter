...
local function PaintFlag(code, x, y, w, h)
    local fn = FLAG_PAINT[code]
...
local function MakeLangPicker(parent, frame)
    local b = vgui.Create("DButton", parent)
    b:SetText("")
    b.hoverFrac = 0
    b.popup     = nil
    ...
    local function ClosePopup()
        if IsValid(b.popup) then b.popup:Remove() end
        b.popup = nil
    end

    b.Paint = function(self, w, h)
        self.hoverFrac = Lerp(FrameTime() * 10, self.hoverFrac, self:IsHovered() and 1 or 0)
        local bg = LerpColor(self.hoverFrac, C.bg_item, C.bg_hover)
        ...
        draw.SimpleText(label, "Trainfitter.Body",
            10 + flagW + 8, h / 2, C.text, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        local arrow = IsValid(self.popup) and "▴" or "▾"
        draw.SimpleText(arrow, "Trainfitter.Body",
            w - 14, h / 2, C.muted, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end

    b.OnRemove = ClosePopup

    b.DoClick = function(self)
        if IsValid(self.popup) then ClosePopup() return end

        local langs = Trainfitter.SupportedLanguages or {}
        local itemH = 30
        local pad   = 6
        local popupH = #langs * itemH + pad * 2
        local popupW = self:GetWide()
        local sx, sy = self:LocalToScreen(0, self:GetTall() + 6)
        ...
        popup.Paint = function(_, w, h)
            draw.RoundedBox(8, 0, 0, w, h, C.bg_panel)
        ...
        for i, code in ipairs(langs) do
            local lang = Trainfitter.Lang and Trainfitter.Lang[code]
            if lang then
                local item = vgui.Create("DButton", popup)
                item:SetText("")
                item:SetSize(popupW - pad * 2, itemH - 2)
                item:SetPos(pad, pad + (i - 1) * itemH)
                item.hoverFrac = 0
                local active = (code == Trainfitter.GetLang())
                item.Paint = function(self2, w, h)
                    self2.hoverFrac = Lerp(FrameTime() * 10, self2.hoverFrac, self2:IsHovered() and 1 or 0)
                    local base = active and C.bg_active or C.bg_item
                ...
                item.DoClick = function()
                    ClosePopup()
                    if code == Trainfitter.GetLang() then return end
                    RunConsoleCommand("trainfitter_lang", code)
        ...
        popup.Think = function(self2)
            if input.IsMouseDown(MOUSE_LEFT) or input.IsMouseDown(MOUSE_RIGHT) then
                local mx, my = gui.MousePos()
                local px, py = self2:GetPos()
                local pw, ph = self2:GetSize()
                local bx, by = b:LocalToScreen(0, 0)
                local bw, bh = b:GetSize()
                local insidePopup = mx >= px and mx <= px + pw and my >= py and my <= py + ph
                local insideBtn   = mx >= bx and mx <= bx + bw and my >= by and my <= by + bh
...
local function BuildInstallView(parent)
    ...
    local function SetSelected(wsid, source)
        ...
        --# по-тихому щупаем не коллекция ли это, и если да - переобуваем кнопку Подтвердить
        Trainfitter.FetchCollectionChildren(wsid, function(children, err)
            ...
            --# коллекцию в избранное не закинешь, это не аддон а список
            favBtn:SetEnabled(false)
            persistBox:SetChecked(false); persistBox:SetEnabled(false)
...
local function BuildFavoritesView(parent)
    ...
    list:AddColumn(Trainfitter.L("col_title"))
    list:AddColumn(Trainfitter.L("col_size")):SetFixedWidth(100)
...
local function BuildSettingsView(parent)
    ...
    --# метрики живут отдельно от тела: их обновление НЕ трогает строки выше, поэтому скролл не скачет
    local metricsRows = {}
    ...
    local function AddMetricLabel(text, font, col, tall, topMargin)
        ...
        lbl:SetZPos(50) --# высокий ZPos = всегда внизу, независимо от порядка создания тела
    ...
    --# обновление метрик с сохранением позиции скролла (тело не трогаем, так что прыжка нет)
    ...
    local function MakeRow(c)
        ...
        --# лейбл берём по текущему языку, ключ = cvar_ + имя без префикса trainfitter_
        ...
        if c.kind == "bool" then
            ...
            cb.OnChange = function(self, val)
                ...
                local newVal = val and "1" or "0"
                if newVal == self._initial then return end
                self._initial = newVal
                Trainfitter.AdminSetConVar(c.name, newVal)
            end
        else
            ...
            entry._initial = c.value or ""
            local function commit(s)
                local v = s:GetValue() or ""
                if v == s._initial then return end
                s._initial = v
                Trainfitter.AdminSetConVar(c.name, v)
            end
            entry.OnEnter = commit
            entry.OnFocusChanged = function(s, gained)
                if not gained then commit(s) end
            end
        end
    ...
    --# клиентские настройки правятся ЛОКАЛЬНО (CreateClientConVar), без сети, доступны всем
    local CLIENT_CVARS = {
    ...
    local function Rebuild(cfg)
        ...
        --# клиентская секция - всегда и для всех (это локальные настройки игрока)
        AddClientSection()

        --# серверная секция - только тем у кого есть manage
        ...
        --# метрики (живут отдельно, всегда снизу через ZPos)
        RefreshMetrics()

        scroll:InvalidateLayout(true)

        --# держим прокрутку на месте при пересборке тела
    ...
    local function ScheduleRebuild()
        ...
        timer.Simple(0.1, function()
            ...
            --# если игрок прямо сейчас печатает в поле внутри настроек - не сносим его ввод, перепроверим позже
    ...
    --# метрики обновляются БЕЗ пересборки тела - вот почему скролл больше не скачет
...
local function BuildLogsView(parent)
    ...
    --# дебаунс: не пересобираем список на каждый символ, а через 0.2с после паузы в наборе
    ...
    --# читаем LogsData напрямую - надёжнее любых конвенций передачи аргументов в table-хук
...
function Trainfitter.OpenMenu()
    ...
    local brandWrap = vgui.Create("DPanel", topBar)
    brandWrap:Dock(LEFT); brandWrap:DockMargin(16, 0, 0, 0); brandWrap:SetWide(280)
    brandWrap.Paint = function() end

    local brandLbl = vgui.Create("DLabel", brandWrap)
    brandLbl:Dock(TOP); brandLbl:SetTall(28)
    ...
    if istable(mods) and #mods > 0 then
        local modLbl = vgui.Create("DLabel", brandWrap)
        modLbl:Dock(TOP); modLbl:SetTall(14)
        modLbl:SetFont("Trainfitter.Caption")
        modLbl:SetText("by SellingVika, modified by " .. table.concat(mods, ", "))
        modLbl:SetTextColor(C.warn)
    else
        local modLbl = vgui.Create("DLabel", brandWrap)
        modLbl:Dock(TOP); modLbl:SetTall(14)
        modLbl:SetFont("Trainfitter.Caption")
        modLbl:SetText("by SellingVika")
        modLbl:SetTextColor(C.muted)
    end

    local menuCloseBtn = MakeCloseButton(topBar)
    menuCloseBtn:SetSize(32, 32)
    menuCloseBtn:Dock(RIGHT); menuCloseBtn:DockMargin(8, 8, 12, 8)
    menuCloseBtn:SetTooltip(Trainfitter.L("close_menu_tooltip"))
    menuCloseBtn.DoClick = function() frame:Close() end

    local closeSep = vgui.Create("DPanel", topBar)
    closeSep:Dock(RIGHT); closeSep:SetWide(1); closeSep:DockMargin(8, 14, 0, 14)
    closeSep.Paint = function(_, w, h)
        surface.SetDrawColor(C.border_soft)
        surface.DrawRect(0, 0, w, h)
    end

    local langPicker = MakeLangPicker(topBar, frame)
    langPicker:Dock(RIGHT); langPicker:DockMargin(0, 10, 8, 10); langPicker:SetWide(160)

    local purgeBtn = MakeFlatButton(topBar,
        Trainfitter.L and Trainfitter.L("purge_client") or "Purge my cache",
        Material("icon16/bin_empty.png"))
    purgeBtn:Dock(RIGHT); purgeBtn:DockMargin(0, 10, 6, 10); purgeBtn:SetWide(170)
    purgeBtn.DoClick = function()
        if Trainfitter.PurgeAllClient then Trainfitter.PurgeAllClient() end
    end

    local activeWrap = vgui.Create("DPanel", topBar)
    activeWrap:Dock(FILL); activeWrap:DockMargin(8, 0, 8, 0)
    activeWrap.Paint = function() end

    local activeRemoveBtn = vgui.Create("DButton", activeWrap)
    activeRemoveBtn:SetText("")
    activeRemoveBtn:SetSize(28, 28)
    activeRemoveBtn:Dock(RIGHT); activeRemoveBtn:DockMargin(8, 10, 0, 10)
    activeRemoveBtn:SetTooltip(Trainfitter.L("remove_active_tooltip"))
    activeRemoveBtn:SetVisible(false)
    activeRemoveBtn.hoverFrac = 0
    activeRemoveBtn.Paint = function(self, w, h)
        self.hoverFrac = Lerp(FrameTime() * 10, self.hoverFrac, self:IsHovered() and 1 or 0)
        local col = LerpColor(self.hoverFrac, C.danger_bg, C.danger_hi)
        draw.RoundedBox(6, 0, 0, w, h, col)
        draw.SimpleText("×", "Trainfitter.H1", w / 2, h / 2 - 2,
    ...
    activeRemoveBtn.DoClick = function()
        local a = Trainfitter.ActiveSkin
        if not a or not a.wsid then return end
        local title = (a.title and a.title ~= "") and a.title or a.wsid
        Derma_Query(
            Trainfitter.L("remove_active_confirm", title),
            Trainfitter.L("remove_active_title"),
            Trainfitter.L("remove"), function()
                if Trainfitter.DeleteSkin then Trainfitter.DeleteSkin(a.wsid) end
            end,
            Trainfitter.L("cancel"), function() end
        )
    end

    local activeLbl = vgui.Create("DLabel", activeWrap)
    activeLbl:Dock(FILL)
    activeLbl:SetFont("Trainfitter.BodyB"); activeLbl:SetTextColor(C.ok)
    activeLbl:SetContentAlignment(6)
    ...
    local function RefreshActive()
        ...
        if mine then
            ...
            if IsValid(activeRemoveBtn) then activeRemoveBtn:SetVisible(true) end
        else
            activeLbl:SetText("")
            activeLbl:SetVisible(false)
            if IsValid(activeRemoveBtn) then activeRemoveBtn:SetVisible(false) end
        end
    end
    RefreshActive()
    ...
    --# Настройки видны всем - там клиентские настройки. Серверная часть внутри гейтится по canManage
